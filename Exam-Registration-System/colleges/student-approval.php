...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    ...
    <table class="table">
        ...
        <tbody id="tbody">
            ...
            $aplications = mysqli_query($db,"SELECT * FROM `application` WHERE `status`='0'");
            ...
            $applications = mysqli_fetch_array($aplications);
            foreach($aplications as $key1)
            {
               
                $select = $key1['sid'];
                $appliedDate = $key1['apply_date'];
                $user = mysqli_query($db, "SELECT * FROM `user` WHERE `id`='$select'");
                ...
                while ($college_row = mysqli_fetch_assoc($user)) {
                    ...
                    <tr>
                        ...
                        <td scope="row">
                            <button type="button" onclick="approveStudent(<?php echo $college_row['id']; ?>)"
...
<script>
    ...
    function approveStudent(id) {
        $.ajax({
            url: "approvalHandle.php",
            type: "POST",
            data: { mode: "approve", studentId: id },
            success: function () {
                location.reload();
            }
        });
